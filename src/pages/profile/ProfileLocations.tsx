...
const ProfileLocations = ({
  ...
  return (
    <div className='flex flex-col items-center pt-5'>
        ...
        <div>
          {languages === 'ko' ?
            ...
            <div>
              ...
              <div>
                Let others know you are in campus by location confirm.
              </div>
              <div>
                Location confirm will last till next day.
              </div>
            </div>
          }
        </div>
      ...
      {locationConfirmed ?
        <Chip color="success" label={languages === 'ko' ? '캠퍼스 위치 확인' : 'Location confirmed'} />
        // <Chips label={'캠퍼스 위치 확인'} className='bg-profile-green' />
        :
        <Chip label={languages === 'ko' ? '캠퍼스 위치 미확인' : 'Location unconfirmed'} />        // <Chips label={'캠퍼스 위치 미확인'} />
      }
      ...
      {!locationConfirmed && location.lat !== 0 && <div>{languages === 'ko' ? '확인 불가' : 'Confirm fail'}</div>}
    </div>
  );
}
