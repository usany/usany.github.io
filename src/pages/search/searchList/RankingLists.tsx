...
function RankingLists({ userObj, userSearch }: Props) {
  ...
  useEffect(() => {
    const membersList = async() => {
      const collectionQuery = query(collection(dbservice, 'members'), orderBy('points', 'desc'), limit(scrollNumber), startAfter(continuing? continuing : ''))
  ...
  const handleScroll = () => {
    if (window.innerHeight + Math.round(document.documentElement.scrollTop) !== document.documentElement.offsetHeight || isLoading) {
    ...
    <>
      ...
      :
        <div>
          ...
          {isLoading && <div className='flex justify-center text-2xl p-28'>loading</div>}
