...
const locationsBuildings = {
  ko: keysArray.map((value) => {
    if (value === 'seall') return [value, '서울캠퍼스 전체']
    if (value === 'gwall') return [value, '광릉캠퍼스 전체']
    return ([value, buildingsObj[value.slice(0, 2)][value].ko.name])
  }),
  en : keysArray.map((value) => {
    if (value === 'seall') return 'All SeoulCampus'
    if (value === 'gwall') return 'All GwangneungCampus'
    return ([value, buildingsObj[value.slice(0, 2)][value].en.name])
  }),
}
