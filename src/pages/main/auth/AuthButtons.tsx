...
import { onSocialClick, onSocialClickMicrosoft, onSocialClickTwitter } from 'src/baseApi/serverbase';

const AuthButtons = () => {
  return (
    <div className='flex flex-col w-screen items-center justify-center px-20'>
      {/* <AuthDialogs /> */}
      <Button className='signin' sx={{ width: '50%', }} startIcon={<img src={staticMicrosoft} className='w-[20px]' />} variant='outlined' onClick={onSocialClickMicrosoft}>
        <div className='truncate'>
          마이크로소프트 로그인
        </div>
      </Button>
      <Button className='signin' sx={{ width: '50%' }} startIcon={<img src={staticGoogle} className='w-[20px]' />} variant='outlined' name='g' onClick={onSocialClick}>
        <div className='truncate'>
          구글 로그인
        </div>
      </Button>
      ...
      <Button className='signin' sx={{ width: '50%' }} variant='outlined' name='h' onClick={onSocialClick}>애플로 로그인</Button>
